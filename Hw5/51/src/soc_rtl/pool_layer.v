...
// MMIO Addresses (Non-buffered - legacy):
...
// ============================================================================
// Non-Buffered Mode (Legacy) - ADD/MUL direct access
// ============================================================================
reg [31:0] legacy_add_dataA, legacy_add_dataB;
reg [31:0] legacy_mul_dataA, legacy_mul_dataB;
reg legacy_add_valid, legacy_mul_valid;
...
always @(posedge clk_i) begin
    if (rst_i) begin
        ...
        if (legacy_add_valid) legacy_add_valid <= 0;
        if (legacy_mul_valid) legacy_mul_valid <= 0;
        
        if (en_i && S == S_IDLE) begin
            // Legacy ADD (0x18, 0x1C)
            if (we_i && addr_i == 32'hC400_0018) begin
                legacy_add_dataA <= data_i;
            end
            else if (we_i && addr_i == 32'hC400_001C) begin
                legacy_add_dataB <= data_i;
                legacy_add_valid <= 1;
            end
            // Legacy MUL (0x24, 0x28)
            else if (we_i && addr_i == 32'hC400_0024) begin
                legacy_mul_dataA <= data_i;
            end
            else if (we_i && addr_i == 32'hC400_0028) begin
                legacy_mul_dataB <= data_i;
                legacy_mul_valid <= 1;
            end
        end
    end
...
// Mux ADD/MUL inputs between buffered and legacy modes
wire add_valid_mux = (S == S_ADD) ? pool_add_data_valid : legacy_add_valid;
wire [31:0] add_dataA_mux = (S == S_ADD) ? pool_add_feed_dataA : legacy_add_dataA;
wire [31:0] add_dataB_mux = (S == S_ADD) ? pool_add_feed_dataB : legacy_add_dataB;

wire mul_valid_mux = (S == S_MUL) ? pool_mul_data_valid : legacy_mul_valid;
wire [31:0] mul_dataA_mux = (S == S_MUL) ? pool_mul_feed_dataA : legacy_mul_dataA;
wire [31:0] mul_dataB_mux = (S == S_MUL) ? pool_mul_feed_dataB : legacy_mul_dataB;
...
floating_point_add ip_pool_add (
    ...
    .s_axis_a_tvalid(add_valid_mux),
    .s_axis_a_tdata(add_dataA_mux),
    
    .s_axis_b_tvalid(add_valid_mux),
    .s_axis_b_tdata(add_dataB_mux),
...
floating_point_mul ip_pool_mul (
    ...
    .s_axis_a_tvalid(mul_valid_mux),
    .s_axis_a_tdata(mul_dataA_mux),
    
    .s_axis_b_tvalid(mul_valid_mux),
    .s_axis_b_tdata(mul_dataB_mux),
