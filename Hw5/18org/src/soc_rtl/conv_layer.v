...
// MMIO Addresses:
//   Weight:  0xC410_0000 (total), 0xC410_0004 (data)
//   Input:   0xC420_0000 (size),  0xC420_0004 (data)
//   Output:  0xC430_0000 (trigger init), 0xC430_0004 (read data)
//   Params:  0xC460_0000 (trigger/status), 0xC460_0004-0x10 (in_w, out_w, in_d, out_d)
//   Non-buffered FMA: 0xC400_000C-0x14 (A, B, C & trigger/read)
// ============================================================================
module conv_layer
#( parameter XLEN = 32 )
(
...
// ============================================================================
// Registers for Conv FMA (convolution): A*B + C - for non-buffered mode
// ============================================================================
reg [31:0] conv_fma_feed_dataA;
reg [31:0] conv_fma_feed_dataB;
reg [31:0] conv_fma_feed_dataC;
reg [31:0] conv_fma_result_reg;

reg conv_fma_data_valid;
wire conv_fma_result_valid;
wire [31:0] conv_fma_result_data;

// ============================================================================
// Conv3D Buffered Mode - BRAM Buffers (same as data_feeder.v)
// ============================================================================
// Weight buffer: max 5x5x12x12 = 3600 (use 4096)
(* ram_style = "block" *) reg [31:0] weight_data [0:4095];
...
// Output image buffer: max 24x24x12 = 6912 (use 2048)
...
// FSM Next State Logic (same as data_feeder.v)
always @(*) begin
    S_NEXT = S;
    case (S)
        S_IDLE: begin
...
// ============================================================================
// Weight Loading (same as data_feeder.v)
// ============================================================================
always @(posedge clk_i) begin
    if (rst_i) begin
        weight_cnt_idx <= 0;
...
// ============================================================================
// Input Image Loading (same as data_feeder.v)
// ============================================================================
always @(posedge clk_i) begin
    if (rst_i) begin
        img_cnt_idx_i <= 0;
...
// ============================================================================
// Output Image Buffer & Accumulation (same as data_feeder.v)
// ============================================================================
always @(posedge clk_i) begin
    if (rst_i) begin
        img_size_o <= 0;
...
// ============================================================================
// Parameter Loading (same as data_feeder.v)
// ============================================================================
always @(posedge clk_i) begin
    if (rst_i) begin
        in_width <= 0;
...
// ============================================================================
// Loop Counting & FMA Control (same as data_feeder.v)
// ============================================================================
always @(posedge clk_i) begin
    if (rst_i) begin
...
// ============================================================================
// Output Logic - Read results (modified for data_feeder style)
// ============================================================================
reg send_flag;
...
always @(posedge clk_i) begin
    if (rst_i) begin
        ...
        // ========== Conv3D Buffered Mode Reads (same as data_feeder.v) ==========
        // Read output image data at 0xC430_0004
