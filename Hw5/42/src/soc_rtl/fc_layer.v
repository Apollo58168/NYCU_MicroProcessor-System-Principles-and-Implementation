...
//   0xC470_0000: Set params (in_size[7:0], out_size[15:8]) and trigger start
//   0xC470_0004: Write weight data
//   0xC470_0008: Write input data
//   0xC470_000C: Read output data
//   0xC470_0010: Read status (0 = done/idle, 1 = busy)
//   0xC470_0014: Write bias data
// MMIO Addresses (Legacy non-buffered):
//   0xC400_0000 (A), 0xC400_0004 (B), 0xC400_0008 (C & trigger/read)
// ============================================================================
module fc_layer
#( parameter XLEN = 32 )
(
...
// ============================================================================
// Weight buffer: max 588 * 10 = 5880 (use 8192)
(* ram_style = "block" *) reg [31:0] weight_data [0:8191];
...
// Bias buffer (reuse some output slots or separate small buffer)
reg [31:0] bias_data [0:31];
...
always @(*) begin
    ...
    case (S)
        S_IDLE: begin
            if (en_i && we_i && addr_i == 32'hC470_0000) begin
                S_NEXT = S_INIT;
            end
        end
        S_INIT: begin
...
always @(posedge clk_i) begin
    ...
    else if (en_i && we_i && addr_i == 32'hC470_0000) begin
        // data_i[15:0] = in_size, data_i[31:16] = out_size
        in_size <= data_i[15:0];
...
always @(posedge clk_i) begin
    if (rst_i) begin
        ...
        if (en_i && we_i && addr_i == 32'hC470_0004 && !weight_written) begin
...
// weight_data - Write Port Only
always @(posedge clk_i) begin
    if (S == S_LOAD_WEIGHT && en_i && we_i && addr_i == 32'hC470_0004 && !weight_written) begin
        weight_data[weight_cnt_idx] <= data_i;
    end
end

// weight_data - Read Port Only
always @(posedge clk_i) begin
...
always @(posedge clk_i) begin
    if (rst_i) begin
        ...
        // Load bias only during dedicated LOAD_BIAS state
        if (en_i && we_i && addr_i == 32'hC470_0014 && !bias_written) begin
...
// bias_data - Write Port Only (separate for BRAM inference)
always @(posedge clk_i) begin
    if (S == S_LOAD_BIAS && en_i && we_i && addr_i == 32'hC470_0014 && !bias_written) begin
        bias_data[bias_cnt_idx] <= data_i;
    end
end
...
// ============================================================================
// Legacy Non-Buffered Mode - FMA direct access
// ============================================================================
reg [31:0] legacy_fma_dataA, legacy_fma_dataB, legacy_fma_dataC;
reg legacy_fma_valid;
...
always @(posedge clk_i) begin
    if (rst_i) begin
        ...
        if (legacy_fma_valid) legacy_fma_valid <= 0;
        
        if (en_i && S == S_IDLE) begin
            // Legacy FMA (0x00, 0x04, 0x08)
            if (we_i && addr_i == 32'hC400_0000) begin
                legacy_fma_dataA <= data_i;
            end
            else if (we_i && addr_i == 32'hC400_0004) begin
                legacy_fma_dataB <= data_i;
            end
            else if (we_i && addr_i == 32'hC400_0008) begin
                legacy_fma_dataC <= data_i;
                legacy_fma_valid <= 1;
            end
        end
    end
...
// Mux FMA inputs between buffered and legacy modes
wire fma_valid_mux = (S == S_MUL) ? fc_fma_data_valid : legacy_fma_valid;
wire [31:0] fma_dataA_mux = (S == S_MUL) ? fc_fma_feed_dataA : legacy_fma_dataA;
wire [31:0] fma_dataB_mux = (S == S_MUL) ? fc_fma_feed_dataB : legacy_fma_dataB;
wire [31:0] fma_dataC_mux = (S == S_MUL) ? fc_fma_feed_dataC : legacy_fma_dataC;
...
floating_point_0 ip_fc_fma (
    ...
    .s_axis_a_tvalid(fma_valid_mux),
    .s_axis_a_tdata(fma_dataA_mux),
    
    .s_axis_b_tvalid(fma_valid_mux),
    .s_axis_b_tdata(fma_dataB_mux),
    
    .s_axis_c_tvalid(fma_valid_mux),
    .s_axis_c_tdata(fma_dataC_mux),
